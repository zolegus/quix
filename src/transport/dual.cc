...
struct plumbing
{
  ...
  {
    transport_type dual( (void*)data, 1, {2} );
  }

  void structors_heap()
  {
    using follow_list_type = transport_type::follow_list_type;
    auto dual1 = std::make_shared< transport_type >( (void*)data, 1, follow_list_type( { 2 } ) );
  }
}
plumbing;
//
...
struct scenarios
{
  using transport_type = transport::dual< uint64_t >;
